...
class TaskConnectionsHelper(object):
    ...
    def sync(self):
        ...
        self.conn_to_set = dict([y_z for y_z in list(self.conn_to_set.items()) if cur_time - y_z[1][3] < self.remove_old_interval])
        self.conn_to_start = dict([y_z1 for y_z1 in list(self.conn_to_start.items()) if cur_time - y_z1[1][2] < self.remove_old_interval])
